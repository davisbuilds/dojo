...
| `change-comprehension` | Optional human comprehension coaching for one change — scope it before implementation, or self-quiz before merge (never scored, never gating) |
...
- `/understand-change` — explain a proposed change's scope, blast radius, and unknowns before it is built
- `/quiz-change` — quiz yourself on an implemented change, one question at a time, before merging
