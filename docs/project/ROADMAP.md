...
| Item | Current State |
|------|---------------|
| Distribution profiles | Contract at revision 11 in `docs/specs/2026-07-27-distribution-profiles-spec.md`; plan at `docs/plans/2026-07-31-distribution-profiles-plan.md`. **Phase 1's measurement layer is shipped** (Tasks 0–5): deterministic listing probes for both harnesses, reviewed profile definitions and harness equivalences, resolution with separated profile/realization identities, per-harness budget policies with a five-shape degradation detector, effective-catalog observation, and byte-identical conformance evidence. All read-only — no installed state has changed. **Contract at revision 12 as of 2026-08-04: the Codex observation was measuring the wrong surface.** `codex debug prompt-input` renders the `codex exec` path and omits account-synced connector plugins that interactive `codex-tui` sessions load. Same directory, model, and minute: 41 entries at 76% via the probe, **110 entries at 178% with every description clipped to ≤77 characters** in the real session — live since at least 2026-07-28. **Task 5A** (inserted, blocks Tasks 6–10) moves observation to the session rollout, which records what the harness actually sent. Remaining in phase 1 after it: routing coverage, cross-machine comparison, the `dojo profiles verify --all` entrypoint, a CI gate, and the first machine-generated position (Tasks 6–10). Phase 2 (apply) is Tasks 11–16 and is an open scope decision. Current position: **Codex interactive 178% and degraded** (pre-dating the 2026-08-04 `vercel` connector disable, which is unverified pending a fresh TUI session); Codex `exec` 76%; Claude Code at 1M 41–58% of 40,000 chars and unaffected, because its probe was built on captured request bodies. |
