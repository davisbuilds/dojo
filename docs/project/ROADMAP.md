...
| Item | Current State |
|------|---------------|
| Distribution profiles | Contract at revision 11 in `docs/specs/2026-07-27-distribution-profiles-spec.md`; plan at `docs/plans/2026-07-31-distribution-profiles-plan.md`. **Phase 1's measurement layer is shipped** (Tasks 0–5): deterministic listing probes for both harnesses, reviewed profile definitions and harness equivalences, resolution with separated profile/realization identities, per-harness budget policies with a five-shape degradation detector, effective-catalog observation, and byte-identical conformance evidence. All read-only — no installed state has changed. Remaining in phase 1: routing coverage, cross-machine comparison, the `dojo profiles verify --all` entrypoint, a CI gate, and the first machine-generated position (Tasks 6–10). Phase 2 (apply) is Tasks 11–16 and is an open scope decision. Current measured position, from the verifier rather than by hand: Codex 4,138 of 5,440 tokens (76%), no degradation; Claude Code at 1M 41–58% of 40,000 chars. Nothing in use is in breach except `viral` at 95% on Codex. |
