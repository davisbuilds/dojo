...
| Item | Current State |
|------|---------------|
| Distribution profiles | Contract at revision 11 in `docs/specs/2026-07-27-distribution-profiles-spec.md`; plan at `docs/plans/2026-07-31-distribution-profiles-plan.md`. **Phase 1's measurement layer is shipped** (Tasks 0–5): deterministic listing probes for both harnesses, reviewed profile definitions and harness equivalences, resolution with separated profile/realization identities, per-harness budget policies with a five-shape degradation detector, effective-catalog observation, and byte-identical conformance evidence. All read-only — no installed state has changed. **Contract at revision 13 as of 2026-08-04. Two compounding errors found and corrected the same day.** (1) The Codex observation measured the wrong surface: `codex debug prompt-input` renders the `codex exec` path and omits account-synced connector plugins that interactive `codex-tui` sessions load. (2) The budget was wrong: two interactive renders of 110 and 56 entries each saturate at **exactly 4,000 tokens**, not the 5,440 that `codex debug models`' 272,000 window implies — a 36% overstatement. Same directory, model, and minute: 41 entries at 76%-of-5,440 via the probe, **110 entries at 246% with every description clipped to ≤77 characters** in the real session, live since at least 2026-07-28. The harness's shortening warning is **not** a fit signal — silent at 144% while clipping 50 of 56 descriptions. **Membership is now the binding constraint on Codex, not just Claude Code at 200k**: the undocumented 31-skill curation is at 100% of budget (over the 90% ceiling by 407 tokens) even with every connector removed, and `full` is 140%; `core` is 47%, `core` plus any one overlay 52–58%, and all twenty `core`-plus-three-overlay combinations fit. **Task 5A** (inserted, blocks Tasks 6–10) moves observation to the session rollout and derives the limit by saturation. Remaining in phase 1 after it: routing coverage, cross-machine comparison, the `dojo profiles verify --all` entrypoint, a CI gate, and the first machine-generated position (Tasks 6–10). Phase 2 (apply) is Tasks 11–16 and is an open scope decision. **Baseline 2026-08-06 (revision 14): 5,468 tokens = 137% of the 4,000 ceiling**, dojo 3,121 (78%), remaining connectors 896, Codex bundled 728, new runtime plugins 723. Two controlled results that day: a local `enabled = false` on an account-synced connector is **inert** (removal in the ChatGPT web app is the only lever), and removing two connectors freed 734 tokens while a Codex desktop update added 723 back — **net 11 tokens**, entry count unchanged at 56, still 50 of 56 descriptions clipped. Claude Code at 1M is unaffected at 41–58%, because its probe was built on captured request bodies. |
