...
def test_build_page_embeds_name_desc_triggers():
    ...
    manifest = {
        "version": 1,
        "skills": [
            {"name": "diagnose", "description": "Debug hard bugs.", "triggers": ["debug this"], "path": "skills/diagnose"},
            {"name": "handoff", "description": "Summarize a session.", "path": "skills/handoff"},
        ],
    }
...
def test_check_detects_drift(tmp_path: Path, monkeypatch):
    ...
    manifest.write_text(json.dumps({"version": 1, "skills": [{"name": "a", "description": "x"}]}), encoding="utf-8")
...
def test_escapes_html_in_descriptions():
    module = load_module()
    manifest = {"version": 1, "skills": [{"name": "x", "description": '<script>alert("xss")</script>'}]}
    page = module.build_page(manifest)
